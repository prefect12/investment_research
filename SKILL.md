...
description: 面向上市公司的 all-in-one 一次性投资研究报告生成技能。用在需要从 0 到 1 研究一家公开交易公司、先把多 agent 研究结果沉淀到本地 research bundle、再组装成 dossier 并生成一份可直接交付的最终报告时使用。覆盖公司历史分期、管理层更替、公开采访与前瞻判断、业务/客户/竞争/财务/资本配置/危机/估值/市场预期差分析，默认输出单文件研究报告。
---
...
6. 直到分阶段门槛完成，再组装 dossier 并生成最终研究文档
...
python3 scripts/render_dossier_report.py --input /tmp/equity-dossiers/tsla/research-bundle/dossier.json
```
...
- `scripts/assemble_dossier_from_bundle.py`
  - 从 bundle 组装 dossier。
- `scripts/render_dossier_report.py`
  - 渲染单文件最终研究报告，作为默认最终交付物。
